...
class ACFFrontendFormAdminNotices {
	...
	public function __construct() {
		$this->notices = array();
		$this->plugin_name = 'Front End Form Extension for ACF';
		$this->plugin_option_prefix = 'acfffef_free_';
	...
	public function show_all() {
		$this->notices = get_option( $this->option_key( 'admin_notices' ) );
		if (is_array( $this->notices )) {
			$notices_cache = $this->notices;
			foreach($notices_cache as $notice) {

				echo $notice[ 'notice' ];
				
				if ( ! $notice['persist'] ) {
					unset( $this->notices[ $notice['key'] ] );
				}
			}

			update_option( $this->option_key( 'admin_notices' ), $this->notices );
			$this->notices = get_option( $this->option_key( 'admin_notices' ) );
